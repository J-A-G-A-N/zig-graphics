...
fn createShaderProgram() !u32 {
    ...
    const vert_src = try std.mem.concat(allocator, u8, &.{ vertex_shader_source, "\x00" });
    ...
    const frag_src = try std.mem.concat(allocator, u8, &.{ fragment_shader_source, "\x00" });
...
fn setupTriangle() void {
    ...
    gl.bufferData(gl.ARRAY_BUFFER, @sizeOf(@TypeOf(triangel_colors)), &triangel_colors, gl.STATIC_DRAW);
